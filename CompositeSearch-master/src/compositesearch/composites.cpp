...
void composites( map<unsigned int, list<unsigned long long int> >& refSubNodes, map<unsigned long long int, geneInfo >& genes, map<pair<unsigned long long int, unsigned long long int>, edgeValues>& edges, map<unsigned long long int, familyInfo >& geneFamilies, unsigned short int MAX_OVERLAP, unsigned short int minCov, unsigned int compositeFamMinSize, unsigned int componentFamMinSize, unsigned int l, string timeInfo)
{
	...
	string fileOut = to_string(l) + ".composites_" + timeInfo;
	ofstream output(fileOut.c_str());
	string fileInfo = to_string(l) + ".compositesinfo_" + timeInfo;
				...
				for(nbD = 1; nbD < NbDomains+1; nbD++)
				{
					...
					else
					{
						...
						else
						{
							...
							unsigned long long int mean = std::floor((float)diff/2.0);
...
void runComposites( map<unsigned long long int, geneInfo >& genes, map<pair<unsigned long long int, unsigned long long int>, edgeValues>& edges, map<unsigned long long int, familyInfo>& geneFamilies, vector<pair<unsigned long long int,unsigned long long int>>& nodesDegree, unsigned short int MAX_OVERLAP, unsigned short int minCov, unsigned int compositeFamMinSize, unsigned int componentFamMinSize, unsigned int nCpu, string timeInfo)
{
	...
	{
		threads.push_back(thread(composites, std::ref(refSubList), std::ref(genes), std::ref(edges), std::ref(geneFamilies), MAX_OVERLAP, minCov, compositeFamMinSize, componentFamMinSize, i, timeInfo));
	}
