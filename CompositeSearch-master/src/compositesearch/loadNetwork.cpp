	...
	else
	{
		...
		while(getline(blastp,hit))
		{
			...
			if(keep == 1)
			{
				...
				localGenes[qseqid] = geneInfo(qlen,false,0,0,0);
...
void runLoadNetwork( string fileIn, string geneList, float pidentLimit, double evalueLimit, map<string, unsigned short int>& positionsList, map<pair<unsigned long long int, unsigned long long int>, edgeValues>& edges, map<unsigned long long int, geneInfo >& genes, vector<pair<unsigned long long int,unsigned long long int>>& nodesDegree, map<unsigned long long int, bool>& geneIsVisited, unsigned int nCpu, string timeInfo)
{
	...
	if( nCpu > 1 )
	{
		...
		string modNameCmd = "for i in $(ls x*); do N=$(($N+1)); mv $i $N.CompositeSearchLoadNetwork_"+ timeInfo +"; done";
		//system("for i in $(ls x*); do N=$(($N+1)); mv $i $N.CompositeSearchLoadNetwork; done");
		system((modNameCmd).c_str());
	}
	else
	{
		// create link
		system(("ln -s " + fileIn + " 1.CompositeSearchLoadNetwork_" + timeInfo).c_str());
	}
	// Multithreading
	vector<thread> threads;
	for(unsigned int i = 1; i < nCpu+1; ++i)
	{
		string subNetwork = to_string(i) + ".CompositeSearchLoadNetwork_" + timeInfo;
