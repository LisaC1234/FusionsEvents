...
void computeFamilies( map<unsigned long long int, geneInfo>& genes, map<pair<unsigned long long int, unsigned long long int>, edgeValues>& edges, map<unsigned long long int, familyInfo>& geneFamilies, map<unsigned long long int, bool>& isVisited, unsigned short int minCov, string outputDir)
{
	// Louvain
	...
	for( it = genes.begin(); it != genes.end(); it++)
	{
		...
		while(!tmpNodes.empty())
		{
			...
			if( isVisited[node] == false )
			{
				...
				while(!currentNodeNeighbors.empty())
				{
					...
					{
						if(checkCoverage(node, neighbor, minCov, genes, edges))
						{
		...
		{
			//info
			geneFamilies[nCC]=familyInfo(1.0,false);
			outputCCinfo << "F" << nCC << "\t" << NbNodes << "\t" << NbEdges << "\t1.00" << endl;
			outputCCnodes << ">F" << nCC << endl;
			while(!ccNodes.empty())
			{
				geneFamilies[nCC].insertGeneID(ccNodes.back());
				genes[ccNodes.back()].setFamilyID(nCC);
		...
		else
		{
			...
			if( goodCovPercent < 100 )
			{
				...
				nCC=louvainCC(refN, refRN, genes, edges, geneFamilies, minCov, nCC, outputCCinfo, outputCCnodes, outputCCedges, refNCCID);
			}
			else
			{
				//
				...
				while(!nodes.empty())
				{
					//cout << "DFS " << nodes.back() << " " << nCC << endl; 
					//
					geneFamilies[nCC].insertGeneID(nodes.back());
					genes[nodes.back()].setFamilyID(nCC);
...
void computeComponentFamilies( vector<unsigned long long int>& components, map<unsigned long long int, vector<unsigned long long int> >& componentFamilies, map<unsigned long long int, geneInfo>& genes)
{
	while( !components.empty() )
	{
		unsigned long long int node = components.back();
		components.pop_back();
		unsigned long long int familyID = genes[node].getFamilyID();
		componentFamilies[familyID].push_back(node);
	}
}
